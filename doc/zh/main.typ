...
Host filesystem:
/data/
├── shared_data/     # 共享数据集（只读或组只读；目录名可配置）
│   ├── ImageNet/
│   ├── COCO/
│   └── ...
...
- 共享只读访问：创建组 `shared_ro`，将需要数据集访问的用户加入该组。共享目录可对组 `shared_ro` 使用 `3775`（setgid + sticky + 组可写，`init-host.sh` 默认）、`2775`（仅 setgid）或 `0750`，视是否需要 sticky 与组内协作写而定。
...
- *常用命令*：`chown` 改属主/组，`chmod` 改模式。新建对象的默认模式受 `umask` 与特殊位规则影响，见下文「八进制、特殊位与 umask」。协作相关的共享模式如 `3775`、`2775` 见「用户与组」。
...
*四位八进制 `chmod SXYZ`*：最左 `S` 为*特殊位*之和——`4` setuid（可执行文件上有效用户常为文件属主）、`2` setgid（可执行文件上有效组；在目录上新建文件/子目录常*继承目录属组*）、`1` sticky（目录上仅文件属主、目录属主或 root 可删/改名他人文件；典型如 `/tmp`）。右侧 `XYZ` 与普通权限相同。例：`3775` = `3`（sticky + 目录 setgid）+ `775`（`rwxrwxr-x`），符号表示中常同时含 `s` 与 `t`。例：`2775` = `2`（仅目录 setgid）+ `775`，常写作 `drwxrwsr-x`。
